...
const updateLeadStatus = async (req, res) => {
  try {
    ...
    const updatedLead = await Lead.findByIdAndUpdate(leadId, { leadStatus: newStatus }, { new: true });

    if (!updatedLead) {
      return res.status(404).json({ message: 'Lead not found.' });
    }

    res.status(200).json({ lead: updatedLead, message: 'Lead status updated successfully.' });
  } catch (error) {
...
export default { index, add, edit, view, deleteData, deleteMany, updateLeadStatus };
